...
#define VISIBILITY_FALLOFF_OPACITY 0.15f
...
static void visibility_polygon_path(const visibility_point_t *polygon, int count)
{
  ...
  nvgMoveTo(state.vg, convert_local_x_to_screen((float)polygon[0].x),
            convert_local_y_to_screen((float)polygon[0].y));
  for (int i = 1; i < count; i++)
  {
    nvgLineTo(state.vg, convert_local_x_to_screen((float)polygon[i].x),
              convert_local_y_to_screen((float)polygon[i].y));
  }
...
static void visibility_draw_shadow(const visibility_point_t *polygon, int count)
{
  ...
  float left = get_translated_x(-500.0f) - margin;
  float right = get_translated_x(500.0f) + margin;
  float top = get_translated_y(-500.0f) - margin;
  float bottom = get_translated_y(500.0f) + margin;

  nvgBeginPath(state.vg);
  nvgFillColor(state.vg, nvgRGBAf(0.0f, 0.0f, 0.0f, layer_opacity));
  nvgMoveTo(state.vg, convert_local_x_to_screen(left), convert_local_y_to_screen(top));
  nvgLineTo(state.vg, convert_local_x_to_screen(left), convert_local_y_to_screen(bottom));
  nvgLineTo(state.vg, convert_local_x_to_screen(right), convert_local_y_to_screen(bottom));
  nvgLineTo(state.vg, convert_local_x_to_screen(right), convert_local_y_to_screen(top));
  nvgClosePath(state.vg);
...
static void visibility_draw_falloff(const visibility_point_t *polygon, int count,
                                    double light_x, double light_y)
{
  if (count < 3)
  {
    return;
  ...
  NVGpaint gradient = nvgRadialGradient(
      state.vg,
      convert_local_x_to_screen((float)light_x),
      convert_local_y_to_screen((float)light_y),
      scale_local_to_screen(100.0f),
      scale_local_to_screen(400.0f),
...
static JSValue js_engine_visibility_set_map(JSContext *ctx, JSValueConst this_val,
                                            int argc, JSValueConst *argv)
{
...
static JSValue js_engine_visibility_draw(JSContext *ctx, JSValueConst this_val,
                                         int argc, JSValueConst *argv)
{
  ...
  visibility_point_t *center_polygon = malloc((size_t)angle_capacity * sizeof(*center_polygon));
  if (!angles || !polygon || !center_polygon)
  {
    free(found);
    free(visible);
    free(corners);
  ...
  for (int sample = 0; sample < VISIBILITY_SAMPLE_COUNT; sample++)
  {
    ...
    if (sample == 0)
    {
      center_count = polygon_count;
      memcpy(center_polygon, polygon, (size_t)polygon_count * sizeof(*polygon));
    }
  }

  visibility_draw_falloff(center_polygon, center_count, light_x, light_y);
  free(found);
  free(visible);
  free(corners);
  free(angles);
